...
for train_idx, test_idx in kf.split(df):
    ...
    mae_cv = mean_absolute_error(actuals, preds)
    correlation_cv = np.corrcoef(actuals, preds)[0, 1] if len(actuals) > 1 else np.nan
    rmse_list.append(rmse_cv)
    mae_list.append(mae_cv)
    correlation_list.append(correlation_cv)

print(f"Cross-Validated RMSEs: {rmse_list}")
print(f"Average Cross-Validated RMSE: {np.mean(rmse_list):.2f}")
print(f"Cross-Validated MAEs: {mae_list}")
print(f"Average Cross-Validated MAE: {np.mean(mae_list):.2f}")
print(f"Cross-Validated Correlations: {correlation_list}")
print(f"Average Cross-Validated Correlation: {np.nanmean(correlation_list):.2f}")
